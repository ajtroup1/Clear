       ...
       if (peek().value().type == TokenType::_exit && peek(1).has_value() && peek(1).value().type == TokenType::open_paren) {
                ...
                if (auto node_expr = parse_expr()) {
                    ...
                    if (peek().has_value() && peek().value().type == TokenType::close_paren) {
                        ...
                        std::cerr << "Expected closing paren after expression" << std::endl;
                        exit(EXIT_FAILURE);
                    }
                    ...
                    if (auto expr = parse_expr()) {
                        ...
                        std::cerr << "Unable to parse expr" << std::endl;
                        exit(EXIT_FAILURE);
                    }
                    if (peek().has_value() && peek().value().type == TokenType::semi) {
                        consume();
                    } else {
                        std::cerr << "Expected semicolon after expressionr" << std::endl;
                        exit(EXIT_FAILURE);
                    }
